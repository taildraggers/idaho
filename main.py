"""Aggregates Idaho-located listings from every taildraggers.com
...
ones whose location ends in ", ID".
...
California/Colorado/Connecticut/Delaware/Florida/Georgia/Hawaii repos'
approach, with the state filter swapped to "ID".
"""
...
PAGE_TITLE = "Taildragger Ads in Idaho"
STATE_ABBR = "ID"
...
_IDAHO_LOCATION_RE = re.compile(r",\s*ID\s*$", re.IGNORECASE)


def _is_idaho(location: str) -> bool:
    return bool(_IDAHO_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        idaho_rows = [row for row in rows if _is_idaho(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(idaho_rows)} in Idaho")
        all_listings.extend(idaho_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Idaho listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Idaho listings: {len(listings)}")
