...
Daily aggregator of Idaho-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "ID".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, ID` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Idaho"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/idaho/`
...
```html
<iframe
  src="https://taildraggers.github.io/idaho/"
  title="Taildragger Ads in Idaho"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_IDAHO_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
